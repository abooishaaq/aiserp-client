            ...
            <FormControlLabel
              control={
                <div className="check-box-pad">
                  <Checkbox onChange={(e) => setSortByName(e.target.checked)} />
                </div>
              }
              label="Name"
            />
        ...
        <div className="check-box">
          <Paper>
            <FormControlLabel
              control={
                <div className="check-box-pad">
                  <Checkbox onChange={(e) => setSortBySrNo(e.target.checked)} />
                </div>
              }
              label="Serial Number"
            />
      ...
      <div className="long-list">
        {profiles
          .filter((profile: any) => {
            ...
            const ok = search
              .map((s) => {
                if (profile.srNo.toLowerCase().includes(s.toLowerCase())) {
                  return true;
                }

                if (profile.name.toLowerCase().includes(s.toLowerCase())) {
                  return true;
                }

                if (
                  profile.fatherName.toLowerCase().includes(s.toLowerCase())
                ) {
                  return true;
                }

                if (
                  profile.motherName.toLowerCase().includes(s.toLowerCase())
                ) {
                  return true;
                }

                if (profile.phone1 && profile.phone1.includes(s)) {
                  return true;
                }

                if (profile.phone2 && profile.phone2.includes(s)) {
                  return true;
                }

                if (profile.address.toLowerCase().includes(s.toLowerCase())) {
                  return true;
                }

                const month = monthNames.map((x) => x.toLowerCase()).indexOf(s);
                const dob = new Date(profile.dob);
                if (dob.getMonth() === month) {
                  return true;
                }

                if (/\d{4}/.test(s) && parseInt(s) === dob.getFullYear()) {
                  return true;
                }
